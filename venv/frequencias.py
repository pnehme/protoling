...
# cálculo dos picos de amplitude > 2sigma para Y
        xbarra = calcMedia(df['a_y'])
        sigma = calcDesvio(df[a_y])
        picoY = []
        for i in df['a_y']:
...
# cálculo dos picos de amplitude > 2sigma para Z
        xbarra = calcMedia(df['a_z'])
        sigma = calcDesvio(df[a_z])
        picoZ = []
        for i in df['a_z']:
